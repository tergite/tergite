...
def _assemble_circuit(
    ...
    with warnings.catch_warnings():
        ...
        config = QasmQobjExperimentConfig(
            n_qubits=num_qubits, memory_slots=memory_slots
        )
    ...
    is_conditional_experiment = any(
        getattr(instruction.operation, "condition", None)
            ...
            if isinstance(ctrl_reg, Clbit):
                ...
                for clbit in clbit_indices:
                    if clbit in ctrl_reg:
                        ...
                        val |= (
                            (ctrl_val >> list(ctrl_reg).index(clbit)) & 1
                        ) << clbit_indices[clbit]
...
def _assemble_pulse_gates(
    ...
    for gate, cals in circuit.calibrations.items():
        for (qubits, params), schedule in cals.items():
            ...
            calibrations.append(
                GateCalibration(
                    str(gate), list(qubits), list(params), qobj_instructions
                )
            )
...
def _extract_common_calibrations(
    ...
    if not (
        experiments and all(hasattr(exp.config, "calibrations") for exp in experiments)
    ):
...
def _assemble_circuits(
    ...
    with warnings.catch_warnings():
        ...
        experiments_and_pulse_libs = parallel_map(
            _assemble_circuit, circuits, [run_config]
        )
    experiments = []
    pulse_library = {}
