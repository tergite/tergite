...
# and a header dictionary.
PulseModule = NewType(
    "PulseModule", Tuple[List[pulse.Schedule], Dict[str, Any], Dict[str, Any]]
)
...
def _experiments_to_circuits(qobj):
    ...
    for exp in qobj.experiments:
        ...
        circuit = QuantumCircuit(
            *quantum_registers, *classical_registers, name=exp.header.name
        )
        ...
        for i in exp.instructions:
                    ...
                    for reg in creg_dict.values():
                        ...
                        if cbit >= size:
                            ...
                            conditional["register"] = (
                                reg if reg.size == 1 else reg[cbit]
                            )
                            break
            ...
            if conditional and name != "bfunc":
                ...
                conditional = {}
        pulse_lib = (
            qobj.config.pulse_library if hasattr(qobj.config, "pulse_library") else []
        )
        ...
        if hasattr(exp.config, "calibrations"):
            circuit.calibrations = {
                **circuit.calibrations,
                **_qobj_to_circuit_cals(exp, pulse_lib),
            }
