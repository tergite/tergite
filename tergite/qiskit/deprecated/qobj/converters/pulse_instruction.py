...
class InstructionToQobjConverter:
    ...
    def __call__(
        ...
        if isinstance(instruction, list):
            ...
            raise QiskitError(
                "Bundle of instruction is not supported except for Acquire."
            )
    ...
    def _convert_play(
        ...
        if isinstance(instruction.pulse, library.SymbolicPulse):
            ...
            command_dict = {
                ...
                "pulse_shape": ParametricPulseShapes.from_instance(
                    instruction.pulse
                ).name,
                "t0": time_offset + instruction.start_time,
                "ch": instruction.channel.name,
...
class QobjToInstructionConverter:
    ...
    def disassemble_value(
        ...
        if isinstance(value_expr, str):
            ...
            value_expr = str_expr(
                **{pname: Parameter(pname) for pname in str_expr.params}
            )
    ...
    def _convert_acquire(
        ...
        if discriminator:
            discriminator = Discriminator(
                name=discriminators[0].name, **discriminators[0].params
            )
        ...
        for acquire_channel, mem_slot, reg_slot in zip(
            acquire_channels, mem_slots, register_slots
        ):
