...
def _assemble(
    ...
    **run_config: Dict,
    ...
    pulse_qobj = any(
        isinstance(exp, (ScheduleBlock, Schedule, Instruction)) for exp in experiments
    )
...
def _parse_pulse_args(
    ...
    **run_config,
    ...
    if backend:
        ...
        if meas_level not in getattr(
            backend_config, "meas_levels", [MeasLevel.CLASSIFIED]
        ):
            raise QiskitError(
...
def _expand_parameters(circuits, run_config):
    ...
    if (
        ...
        circuits = [
            circuit.assign_parameters(binds)
