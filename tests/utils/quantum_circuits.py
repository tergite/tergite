...
def remove_idle_qubits(circuit:QuantumCircuit)-> QuantumCircuit:
    ...
    qubit_indices = {qubit: idx for idx, qubit in enumerate(sorted(active_qubits, key=lambda q: circuit.find_bit(q).index))}
    clbit_indices = {clbit: idx for idx, clbit in enumerate(sorted(active_clbits, key=lambda c: circuit.find_bit(c).index))}
    ...
    if circuit.qregs or circuit.cregs:
        ...
        new_circuit = QuantumCircuit(*qregs, *cregs, name=circuit.name, global_phase=circuit.global_phase)
